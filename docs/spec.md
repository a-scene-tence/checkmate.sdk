...
- **방식**: 설정 탭 → "데이터 백업/복원"
  - **📤 JSON으로 내보내기** (파일 다운로드 우선, 환경 인지형):
    1. 일반 브라우저(데스크톱·모바일·Vercel) → `<a download>` blob으로 `.json` 파일을 기기에 직접 저장 (기본 동작)
    2. AiT WebView 등 다운로드 미지원 → `navigator.share`(파일)로 네이티브 공유 시트 (파일앱 저장 등) — AiT WebView는 blob 다운로드가 조용히 실패하므로(§7.6) 1단계를 건너뜀
    3. 최종 폴백(공유도 미지원) → 텍스트 복사 모달
    - AiT WebView 감지: 네이티브 셸이 주입하는 `window.__GRANITE_NATIVE_EMITTER`/`__CONSTANT_HANDLER_MAP` 전역으로 판별 (`isAitWebView()`)
  - **📥 JSON 파일에서 불러오기**: 파일 선택 → 업로드 복원 (기존 동작 유지)
  - **📋 텍스트로 복원**: 내보낸 JSON 텍스트를 붙여넣어 복원 (모바일 복사 백업 대응)
- **기기 이전 시**: 내보내기(파일 다운로드/공유) → 새 기기에서 파일 불러오기 또는 텍스트 붙여넣기 복원
...
- **현재 캐시 버전**: `checkmate-v11` (2026-06-06에 v10 → v11 증가 — JSON 내보내기 파일 다운로드 우선 전환으로 캐시 강제 갱신)
...
| 2026-06-06 | JSON 내보내기를 **파일 다운로드 우선**으로 전환: 일반 브라우저는 `<a download>` blob으로 `.json` 파일 직접 저장, AiT WebView(blob 다운로드 미지원, §7.6)는 `isAitWebView()`로 감지해 공유 시트→텍스트 복사 폴백. SW v11 | — |
