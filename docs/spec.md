...
- **공동 자산**: 별도 분리 관리
- 자산별 잔액 실시간 집계
- **매도(현금화)**: 자산 잔액을 계좌/현금 수입(`assetId` 포함 income tx)으로 기록 → 계좌잔고 상승. 매도액은 **"현재 여유자금"에도 가산**(적립으로 커밋됐던 자산의 유동화 복원, `calcPersonalRemain`의 `assetSaleIncome`). 이중 반영 방지를 위해 `overBudget`는 자산 매도(assetId) 수입을 초과지출 상쇄에서 제외.
...
- 거래 검색
- 기간별 합산 및 집계
- **금액 계산식 입력**: 금액칸에 사칙연산(`+ − × ÷`·괄호·소수)을 직접 입력 가능. 예: 카드 할인 차감 시 기입력 `50,000`을 `50000-5000`으로 고쳐 입력 → blur/Enter/저장 시 `45,000`으로 자동 계산. 입력칸 아래 `= ₩…` 실시간 미리보기 + 연산자 보조 버튼(`− + × ÷ ⌫`, 모바일 숫자패드 대응). 안전 평가(`eval` 미사용, 토큰화→shunting-yard). 추가·수정 모달 공통.

### 5.3 예산 관리
- 월별 카테고리별 예산 설정
- 진행률 바 시각화 (실시간)
- 초과 시 알림
- **이번 달 조정**(고급 설정 `.sinp-comma-adj`): 해당 월 예산에 더할/뺄 예외 금액(부호 `+/-`). **계산식 입력 지원** — 선행 부호 외 사칙연산이 있으면 수식으로 인식해 `evalExpr`로 자동 계산(예: `523490-10000` → blur 시 `+513,490`). 부호만 있는 값(`+523,490`·`-5000`)은 부호 모드로 콤마 포맷 + **커서 위치 유지**(중간 숫자 수정 시 캐럿이 맨 뒤로 튀지 않음).
...
- **현재 캐시 버전**: `checkmate-v13` (2026-06-06에 v12 → v13 증가 — 자산 매도 시 여유자금 반영 버그 수정으로 캐시 강제 갱신)
...
| 2026-06-06 | 활성 세그먼트 토글 버튼(`.tb.active`) 강조 강화: 기존 헤어라인+굵게에 **밑줄**(text-underline-offset 3px) 추가, SW 캐시 v10 | — |
| 2026-06-06 | 거래 금액칸 **계산식 입력** 추가(카드 할인 차감 등): 사칙연산 직접 입력→자동 계산(`evalExpr` 안전 평가), `= ₩…` 미리보기 + 연산자 보조 버튼, 추가·수정 모달 공통, SW 캐시 v11 | — |
| 2026-06-06 | "이번 달 조정" 칸(`.sinp-comma-adj`) **계산식 입력**(`evalExpr` 재사용, 선행 부호 제외 판별 `hasMathOpSigned`, blur 확정) + **커서 위치 유지 버그 수정**(맨 뒤 이동 → 편집 위치 보존), SW 캐시 v12 | — |
| 2026-06-06 | **자산 매도 시 여유자금 미반영 버그 수정**: 매도(assetId income)액을 `calcPersonalRemain`의 `remain`에 가산(`assetSaleIncome`), `overBudget`에서 매도 수입 제외로 이중 반영 방지, 대시보드 노트 추가, SW 캐시 v13 | — |
