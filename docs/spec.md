...
- **방식**: 설정 탭 → "데이터 백업/복원"
  - **📤 JSON으로 내보내기** (환경 인지형):
    1. 모바일/WebView → `navigator.share`(파일)로 네이티브 공유 시트 (파일앱 저장, 메신저 전송 등)
    2. 데스크톱 브라우저 → 기존 `<a download>` 파일 다운로드
    3. 폴백(공유 미지원 WebView) → 텍스트 복사 모달
  - **📥 JSON 파일에서 불러오기**: 파일 선택 → 업로드 복원 (기존 동작 유지)
  - **📋 텍스트로 복원**: 내보낸 JSON 텍스트를 붙여넣어 복원 (모바일 복사 백업 대응)
- **기기 이전 시**: 내보내기(공유/복사) → 새 기기에서 파일 불러오기 또는 텍스트 붙여넣기 복원
